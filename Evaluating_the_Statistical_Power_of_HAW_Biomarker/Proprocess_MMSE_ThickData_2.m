function [AD_data,SMC_data,LMCI_data,EMCI_data,CN_data] = Proprocess_MMSE_ThickData_2(Information_new_path,Thickness_path,Subject_path)
...
for i=1:size(PTID)
    ...
    [~, I]=min(cell2mat(temp(:,5)));
    [~, JJ]=max(cell2mat(temp(:,5)));
    mmse_exist=cell2mat(temp(I,14));
    while (~isnumeric(mmse_exist))&&(I<size(temp,1))
        I=I+1;
        mmse_exist=cell2mat(temp(I,14));
    end
    mmse_j_exist=cell2mat(temp(JJ,14));
    while (~isnumeric(mmse_j_exist))&&(JJ>1)
        JJ=JJ-1;
        mmse_j_exist=cell2mat(temp(JJ,14));
    end
    if (I<30)&&(JJ>0)
    ...
    %     mmse_new_r=mmse_new(X,15);
        disease=temp(I,6);
        if ~isempty(temp_vector)&&isnumeric(cell2mat(mmse))&&isnumeric(cell2mat(mmse_J))
            switch char(disease)
                case 'AD'
