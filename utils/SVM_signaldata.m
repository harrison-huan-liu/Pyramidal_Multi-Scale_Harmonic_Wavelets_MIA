function [accuracy, sensitivity, specificity, f_score, Beta] = SVM_signaldata(CN_signdata, AD_signdata)
    %% 10-fold
    CN_num = size(CN_signdata, 2);
    AD_num = size(AD_signdata, 2);
    sample_num = CN_num + AD_num;
    ...
    for loop = 1:10
        ...
        % Ensure confusion matrix has both classes present
        if size(cm, 1) == 2
            ...
            accuracy(loop) = NaN;
            sensitivity(loop) = NaN;
            specificity(loop) = NaN;
            f_score(loop) = NaN;
        end
